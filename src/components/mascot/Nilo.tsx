...
export function Nilo({
  ...
  const eye = emotion === "sleepy" ? "sleep" : "open";
  const look = emotion === "thinking" || emotion === "confused" ? -4 : emotion === "curious" ? 3 : 0;
  const leafDrop = emotion === "confused" || emotion === "encourage" ? 10 : 0;
  const uid = `nilo-${size}-${emotion}`;
  return (
    <div className={`nilo-float ${className}`} style={{ width: size, height: size }} aria-hidden>
      <svg viewBox="0 0 160 160" width={size} height={size}>
        ...
        <ellipse cx="80" cy="148" rx="42" ry="8" fill="#1a2744" opacity="0.18" />
        <circle cx="80" cy="86" r="62" fill={`url(#${uid}-glow)`} opacity="0.5" />
        <g filter={`url(#${uid}-soft)`} className={emotion === "celebrate" ? "pulse-soft" : ""}>
          <g transform={`rotate(${leafDrop} 110 40)`}>
          ...
          {eye === "open" ? (
            <>
              ...
              <circle cx={60.5 + look} cy="82" r="1" fill="#fff" opacity="0.5" />
            </>
          ) : (
            <>
